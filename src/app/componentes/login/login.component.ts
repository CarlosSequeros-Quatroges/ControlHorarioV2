...
export class LoginComponent {
  ...
  ngOnInit(): void {
    ...
    if (this.localStorageService.getItem('email')) {
      this.loginForm
        .get('email')
        ?.setValue(String(this.localStorageService.getItem('email')));
  ...
  validar() {
    ...
    this.auth.login(login).subscribe(
      (resp) => {
        ...
        var newdate: string =
          date1[2] + '-' + date1[1] + '-' + date1[0] + ' ' + datehour[1];
        ...
        this.auth.guardarToken(
          this.usuario.token_api,
          this.usuario.valido_hasta,
        );
