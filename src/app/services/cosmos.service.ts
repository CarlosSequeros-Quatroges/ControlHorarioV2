...
export class CosmosService {
  ...
  recuperaRegistrosDia (): Observable<RespRegistros> {
    ...
    return this.http.get<RespRegistros>(`${this.url}/recupera-registros-dia?email=${usuario.email}&empresa=${usuario.codigo}&matricula=${usuario.matricula}`, {headers} )
    .pipe(
        map( (resp) => {
  ...
  recuperaRegistrosMes (mes: string): Observable<RespRegistros> {
    ...
    .append('Access-Control-Allow-Origin', '*')
    .append('Access-Control-Allow-Headers', 'origin, content-type, accept,authorization,securityToken,access-control-allow-origin')
    return this.http.get<RespRegistros>(`${this.url}/recupera-registros-mes2?email=${usuario.email}&empresa=${usuario.codigo}&matricula=${usuario.matricula}&mes=${mes}`, {headers} )
    .pipe(
        map( (resp) => {
          return resp;
        }),
  ...
  recuperaIncidenciasMes (mes: string): Observable<RespIncidencias> {
    ...
    return this.http.get<RespIncidencias>(`${this.url}/recupera-incidencias-mes?email=${usuario.email}&empresa=${usuario.codigo}&matricula=${usuario.matricula}&mes=${mes}`, {headers} )
    .pipe(
        map( (resp) => {
          return resp;
        }),
  ...
  actualizaRegistro (usuario: UsuarioModel,registro: Registro): Observable<String> {
    ...
    return this.http.post<RespBase>(`${this.url}/actualiza-registro`,data, {headers} )
    .pipe(
      map( (resp) => {
  ...
  actualizaRegistros (usuario: UsuarioModel,registros: Registro[]): Observable<RespActualizaRegistros> {
    ...
    return this.http.post<RespActualizaRegistros>(`${this.url}/actualiza-registros`,data, {headers} )
    .pipe(
      map( (resp) => {
